...
namespace InteractiveTango {
    ...
    protected:
        ...
        float timeSinceStoppedCircling;
        ...
        float lastVolValueSent = 0;
        
        int shouldSendContinuous = 0;
        int everyXSample = 10;
    ...
    public:
        ...
        CirclingGestures(RecognizedGestures *grecognition, BeatTiming *timer, SignalAnalysis *fiv, int wd) : GestureToMusic(grecognition, timer)
        {
            timeSinceStartedCircling = 0;
            timeStartedCircling = -1;
            filteredInput = (Filter *)fiv;
            
            //create new motion data
            motionData.push_back(new MotionAnalysisEvent(MotionAnalysisDataType::DoubleEvent, 0));
            motionData[0]->setName("Time Circling");
            
            //create data for input accel
            motionData.push_back(new MotionAnalysisEvent(MotionAnalysisDataType::DoubleEvent, 0));
            motionData[1]->setName("Accel 15Hz LP Filter");
            
            mtimer = timer;
            
            updateMaxCircling();
            
            filteredInput->calcMotionData(true);
            whichDancer = wd;
            
            shouldSendContinuous = 0;
            
        };
        
        void updateMaxCircling()
        {
            //guesssessssss.......
            minTimeCircling = 0;
            maxTimeCircling = mtimer->getPulse8() * 6.0 ; //default 2 bars
        ...
        virtual void update(float seconds = 0)
        {
            ...
            if( curGesture->getCurGesture() == TangoGestures::CIRCLING )
            {
                ...
                timeSinceStoppedCircling = - 1;
                timeSinceStartedCircling = seconds - timeStartedCircling;
                if( timeSinceStartedCircling > maxTimeCircling )
                {
                    timeSinceStartedCircling = maxTimeCircling;
                }
            ...
            {
                if( timeSinceStartedCircling > 0 && timeSinceStoppedCircling!=-1 )
                {
                    timeSinceStartedCircling -= (seconds - (timeSinceStoppedCircling))/2.0;
                }
                else
                {
                    timeSinceStoppedCircling = seconds;
                }
                ...
                if ( stoppedCircling )
                {
                    if(_COUT_DEBUG_) std::cout << "STOPPED CIRCLING!\n";
                }
            }
        ...
        std::vector<ci::osc::Message> getOSC()
        {
            ...
            if( stillPlay )
            {
                if( shouldSendContinuous % everyXSample == 0 )
                {
                    msg.setAddress(CIRCLING_VOLUME);
                    float val = motionData[0]->asFloat() / maxTimeCircling;
                    if(val < 0) val = 0;
                    msg.addFloatArg( val );
                    lastVolValueSent = val;
                    //            std::cout << "circling volume: " << val << "time since started circling: " << timeSinceStartedCircling << std::endl;
                    
                    gMsg.setAddress(CIRCLING_ACCEL);
                    for(int i=0; i<5; i++)
                        gMsg.addFloatArg( filteredInput->getMotionData().at(i)->scaledValue()) ;
                    
                    msgs.push_back(msg);
                    msgs.push_back(gMsg);
                    
                    shouldSendContinuous = 1;
                    
                }
                else
                {
                    shouldSendContinuous++;
                }
            }
            else if( lastVolValueSent != 0 )
            {
                lastVolValueSent = 0;
                msg.setAddress(CIRCLING_VOLUME);
                msg.addFloatArg(lastVolValueSent);
                msgs.push_back(msg);
    ...
    public:
        ...
        virtual void update(float seconds = 0)
        {
            ...
            stillPlay = sendPlay = changedToBoleo;
        ...
        std::vector<ci::osc::Message> getOSC()
        {
            ...
            //            {
            //            
            //                std::cout << "Should send play for boleo\n";
            //                
            //            }
